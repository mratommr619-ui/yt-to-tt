...
// --- [၂] Downloader with Always Update Feature ---
function downloadVideo(url, output) {
    console.log("🔄 Updating yt-dlp to latest version...");
    try {
        // GitHub Actions ထဲမှာ yt-dlp ကို အရင် update လုပ်မယ်
        execSync(`pip install -U yt-dlp`, { stdio: 'inherit' });
    } catch (e) {
        console.log("⚠️ yt-dlp update failed, using pre-installed version.");
    }

    console.log(`📥 Downloading: ${url}`);
    try {
        // --no-check-certificate နဲ့ user-agent သုံးပြီး ဒေါင်းမယ်
        execSync(`yt-dlp --no-check-certificate --user-agent "Mozilla/5.0" "${url}" -o "temp_raw.mp4"`, { stdio: 'inherit' });
    } catch (e) {
        console.log("⚠️ yt-dlp failed, trying CURL backup for Drive...");
        let finalUrl = url;
...
// --- [၃] TikTok Upload Logic (Stable Post Button) ---
async function uploadToTikTok(videoPath, caption) {
    ...
    try {
        ...
        if (page.url().includes('login')) {
            console.log("🔑 Session expired. Logging in with Username/Password...");
            const user = process.env.TIKTOK_USERNAME;
            const pass = process.env.TIKTOK_PASSWORD;
            if (!user || !pass) throw new Error("No Credentials Found!");
        ...
        console.log("⌛ Waiting for Processing (60s)...");
        ...
        try {
            await page.waitForFunction(() => {
                ...
                return btns.find(b => b.innerText.includes('Post') || b.textContent.includes('Post'));
            }, { timeout: 30000 });
            ...
            console.log("⚠️ Selector Fallback Used...");
