...
TEXTS = {
    'my': {
        ...
        'expired': (
            "⚠️ **သင့်ရဲ့ Free Trial ၂၄ နာရီ ကုန်ဆုံးသွားပါပြီ**\n\n"
            "ဆက်လက်သုံးရန် Premium ဝယ်ယူနိုင်ပါတယ်။\n"
            f"💰 **Price:** ၃၀၀၀ ကျပ် / 1.0 USDT\n\n"
            f"💳 **KPay/AYA:** `{KPAY_NO}`\n"
            f"🌐 **USDT:** `{USDT_ADDRESS}`"
        )
    },
    'en': {
        ...
        'expired': "⚠️ **Trial Expired!** Upgrade to Premium for 3000 MMK / 1.0 USDT."
    }
}
...
async def handle_input(c, m):
    ...
    # User အချက်အလက် မရှိသေးရင် (ဘာသာစကား မရွေးရသေးရင်)
    if not u_doc_ref.exists:
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("🇲🇲 မြန်မာစာ", callback_data="lang_my"),
            InlineKeyboardButton("🇺🇸 English", callback_data="lang_en")
        ]])
        await m.reply_text("⚠️ ကျေးဇူးပြု၍ ဘာသာစကား အရင်ရွေးချယ်ပေးပါ / Please select language first.", reply_markup=kb)
        return

    u_doc = u_doc_ref.to_dict()
    lang = u_doc.get('lang', 'my')

    # Expiry Check (Error မတက်အောင် get နဲ့ စစ်မယ်)
    expiry_str = u_doc.get('expiry_date')
    if expiry_str and datetime.now() > datetime.strptime(expiry_str, "%Y-%m-%d %H:%M:%S"):
...
async def steps_handler(c, m):
    ...
    curr = user_steps[uid]
    step, lang = curr['step'], curr['lang']
    
    if step == 'name':
        user_steps[uid]['data']['name'] = "Movie" if m.text == "/skip" else m.text
        user_steps[uid]['step'] = 'len'; await m.reply_text(TEXTS[lang]['ask_len'])
    elif step == 'len':
        try:
            val = 5 if m.text == "/skip" else int(m.text)
            user_steps[uid]['data']['len'] = val
            user_steps[uid]['step'] = 'wm'; await m.reply_text(TEXTS[lang]['ask_wm'])
